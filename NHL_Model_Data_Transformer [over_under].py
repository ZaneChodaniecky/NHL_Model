# -*- coding: utf-8 -*-
"""
Created on Thu Oct 10 10:26:45 2024
...
def transform_data(fileName):
    ...
    # Update win history file (Cant run on Franklin Network)
    ...
    df_filtered = df_filtered.query("situation == 'all' & playoffGame == 0 & season >= 2020")
    ...
    df_trimmed = pd.merge(df_filtered,df_win_history[['gameId','home_win']], on='gameId', how='left')
    
    
    # Create custom columns
    outcomeConditions = [
        ((df_trimmed['home_or_away'] == 'HOME') & (df_trimmed['home_win'] == 1)),
    ...
    df_trimmed.loc[:,'goalsTotal'] = df_trimmed['goalsFor'] + df_trimmed['goalsAgainst']
    
    # If game regulation ends in tie, determine the OT/shootout winner and add 1 goal for them
    df_trimmed.loc[(df_trimmed['tie'] == 1) & (df_trimmed['home_or_away'] == 'HOME') & (df_trimmed['home_win'] == 1), 'goalsFor'] += 1
    df_trimmed.loc[(df_trimmed['tie'] == 1) & (df_trimmed['home_or_away'] == 'HOME') & (df_trimmed['home_win'] == 0), 'goalsAgainst'] += 1
    df_trimmed.loc[(df_trimmed['tie'] == 1) & (df_trimmed['home_or_away'] == 'AWAY') & (df_trimmed['home_win'] == 0), 'goalsFor'] += 1
    df_trimmed.loc[(df_trimmed['tie'] == 1) & (df_trimmed['home_or_away'] == 'AWAY') & (df_trimmed['home_win'] == 1), 'goalsAgainst'] += 1
    ...
    df_train_data.to_csv('NHL_Data_Over_Under_Transformed.csv',index=False)
    ...
    df_predict_data.to_csv('NHL_Data_Over_Under_Predict.csv',index=False)
