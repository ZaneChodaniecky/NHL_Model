...
def transform_data(fileName):
    ...
    df_home_goalie_stats = df_goalie_stats.query(f"lastGoalieInNet == 1 & isHomeTeam == 1 & season >= {START_YEAR}")
    df_away_goalie_stats = df_goalie_stats.query(f"lastGoalieInNet == 1 & isHomeTeam == 0 & season >= {START_YEAR}")
    ...
    homeShotDataMissing = df_home_combined['goalieIdForShot'].isna().sum()
    awayShotDataMissing = df_away_combined['goalieIdForShot'].isna().sum()
    
    if homeShotDataMissing != 0 or awayShotDataMissing != 0:
    ...
    df_home_combined.dropna(subset=['goalieIdForShot'],inplace=True)
    df_away_combined.dropna(subset=['goalieIdForShot'],inplace=True)
